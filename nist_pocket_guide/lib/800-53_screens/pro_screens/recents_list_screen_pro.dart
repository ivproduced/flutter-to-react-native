...
class RecentControlsScreen extends StatelessWidget {
  final PurchaseService purchaseService;
  const RecentControlsScreen({super.key, required this.purchaseService});

  @override
  Widget build(BuildContext context) {
    final recentControls =
        AppDataManager().recentControlIds
            .map((id) => AppDataManager().getControlById(id))
            .whereType<Control>() // filters out nulls
            .toList();

    return Scaffold(
      appBar: AppBar(title: const Text('Recent Controls')),
      body:
          recentControls.isEmpty
              ? const Center(child: Text('No recent controls yet.'))
              : ListView.builder(
                itemCount: recentControls.length,
                cacheExtent: 1000.0,
                itemBuilder: (context, index) {
                  final control = recentControls[index];
                  return ControlTile(
                    control: control,
