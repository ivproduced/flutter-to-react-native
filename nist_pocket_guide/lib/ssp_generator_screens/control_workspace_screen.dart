...
// Import for the new screen (you'll create this file and uncomment the import)
...
class _ControlSspWorkspaceScreenState extends State<ControlSspWorkspaceScreen> {
  ...
  void initState() {
    ...
    if (kDebugMode) {
      print(
        'DEBUG: ControlSspWorkspaceScreen initState called for controlId: [33m[1m[4m[7m${widget.controlId}[0m',
      );
    }
  ...
  Future<void> _loadData() async {
    if (kDebugMode) {
      print(
        'DEBUG: _loadData called for controlId: [33m${widget.controlId}[0m',
      );
    }
    ...
    if (_system != null && _control != null) {
      ...
      for (var objPart in _control!.flatAssessmentObjectives) {
        ...
        // Defensive assignment: isMet is only true if explicitly true
        final isMetValue = existingResp?.isMet == true;
        if (kDebugMode) {
          print(
            'DEBUG _loadData: For objectiveKey: '
            '[33m$key[0m, existingResp?.isMet: [36m${existingResp?.isMet}[0m, will set isMet: [35m$isMetValue[0m',
          );
        }
        ...
        _objectiveResponses.add(
          AssessmentObjectiveResponse(
            ...
            isMet: isMetValue, // Default to false unless explicitly true
          ),
        );
      }
    }
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            // Removed explicit save button as auto-save on exit is handled via WillPopScope
            TextButton(
                ...
                child: SegmentedButton<SspWorkspaceViewMode>(
                  ...
                  segments: <ButtonSegment<SspWorkspaceViewMode>>[
                    ButtonSegment<SspWorkspaceViewMode>(
                      ...
                      label: Text('Statement', style: TextStyle(fontSize: 13)),
                      icon: Icon(Icons.description_outlined, size: 18),
                    ),
                    ButtonSegment<SspWorkspaceViewMode>(
                      value: SspWorkspaceViewMode.assessmentChecklist,
                      label: Text('Checklist', style: TextStyle(fontSize: 13)),
                  ...
                  onSelectionChanged: (Set<SspWorkspaceViewMode> newSelection) {
                    _setStateIfMounted(() {
        ...
        bottomNavigationBar: StatementNavBar(
          enhancements: _control?.enhancements ?? [],
          control: _control!,
  ...
  Widget _buildObjectiveStatementsList() {
    ...
    // Add a single instructional statement below the label
    sectionChildren.add(
      Padding(
        padding: const EdgeInsets.fromLTRB(16.0, 0, 16.0, 8.0),
        child: Text(
          "Select an objective to provide implementation details or review its requirements.",
          style: Theme.of(
            context,
            ...
            return Card(
              ...
              child: ListTile(
                ...
                title: Column(
                  ...
                  children: [
                    Text(
                      'Objective ID: ${objectivePart.id ?? objectiveKey}',
                    ...
                    // Show placeholder completion progress if LLM data and placeholders exist
                    if (llmObjectiveStatement != null &&
                        llmObjectiveStatement.placeholders.isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(top: 2.0),
                        child: Builder(
                          builder: (context) {
                            final total =
                                llmObjectiveStatement!.placeholders.length;
                            final userValues =
                                _controlImplementation
                                    .llmObjectivePlaceholderValues[objectiveKey] ??
                                {};
                            final filled =
                                llmObjectiveStatement.placeholders.where((ph) {
                                  final val =
                                      userValues[ph.label] ??
                                      userValues['[${ph.label}]'] ??
                                      '';
                            ...
                            return Text(
                              'Placeholders: $filled/$total filled',
                              style: Theme.of(
                                context,
  ...
  Widget _buildAssessmentChecklistSection() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (kDebugMode) {
          print(
            "DEBUG_ItemBuilder: objResp.objectiveKey: [33m${objResp.objectiveKey}[0m, objResp.isMet: [35m${objResp.isMet}[0m",
          );
        }
              ...
              children: [
                ...
                SwitchListTile.adaptive(
                  ...
                  onChanged: (bool? newValue) {
                    if (kDebugMode) {
                      print(
                        'DEBUG: SwitchListTile.onChanged for ${objResp.objectiveKey} - newValue: [36m$newValue[0m, old isMet: [35m${objResp.isMet}[0m',
                      );
                    }
                    if (newValue != null) {
                      // --- ENFORCE PLACEHOLDER COMPLETION ---
                      bool allowToggle = true;
                      String? unmetPlaceholderMsg;
                      // Find LLM statement for this objective
                      LlmObjectiveStatement? llmObjectiveStatement;
                      if (_llmControlData != null) {
                        try {
                          llmObjectiveStatement = _llmControlData!
                              ...
                              .firstWhere(
                                (stmt) =>
                                    stmt.objectiveId == objResp.objectiveKey,
                              );
                        } catch (_) {}
                      }
                      if (newValue) {
                        // Only allow ON if all placeholders are filled (if any exist)
                        if (llmObjectiveStatement != null &&
                            llmObjectiveStatement.placeholders.isNotEmpty) {
                          final userValues =
                              _controlImplementation
                                  .llmObjectivePlaceholderValues[objResp
                                  .objectiveKey] ??
                              {};
                          final total =
                              llmObjectiveStatement.placeholders.length;
                          ...
                          if (filled < total) {
                            allowToggle = false;
                            unmetPlaceholderMsg =
                                "All placeholders must be filled before marking this objective as met.";
                          }
                        }
                      }
                      if (allowToggle) {
                      ...
                      } else if (unmetPlaceholderMsg != null) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text(unmetPlaceholderMsg)),
                        );
                      }
                    }
                  },
